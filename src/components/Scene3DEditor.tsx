...
// 3D Object Component
const Object3D = ({ object, isSelected, onSelect }) => {
...
// Scene Component
const Scene = ({ objects, selectedObject, onSelect, transformMode }) => {
  return (
    <>
...
const Scene3DEditor = () => {
  ...
  const objectTypes = [
    ...
    { type: 'torus', icon: RotateCcw, name: 'Torus' },
      ...
      <div className="container mx-auto px-4">
        ...
        >
          <Card className="bg-black/20 backdrop-blur-xl border-cyan-500/20">
            <CardHeader>
              <CardTitle className="text-white">Scene Objects</CardTitle>
              <CardDescription className="text-gray-300">
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center">
                        {(() => {
                          const IconComponent = objectTypes.find(t => t.type === object.type)?.icon;
                          return IconComponent ? <IconComponent className="w-4 h-4 mr-2 text-purple-400" /> : null;
                        })()}
                        <span className="text-white font-medium capitalize">{object.type}</span>
                      </div>
                    ...
                    <div className="text-sm text-gray-400">
                      Scale: [{object.scale.map(s => s.toFixed(1)).join(', ')}]
                    </div>
                  </div>
                ))}
              </div>
